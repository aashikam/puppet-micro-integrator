...
# Class: mi_dashboard
# Init class of Micro Integrator Dashboard
class mi_dashboard inherits mi_dashboard::params {
  ...
  */
  ...
  file { "${install_path}/${start_script_template}":
    ...
    group   => $user_group,
    mode    => '0754',
    content => template("${module_name}/dashboard-home/${start_script_template}.erb")
  }
  ...
  file { "${install_path}/${deployment_toml_template}":
    ...
    group   => $user_group,
    mode    => '0644',
    content => template("${module_name}/dashboard-home/${deployment_toml_template}.erb")
  }
